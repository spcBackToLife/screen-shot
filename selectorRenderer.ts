...
function init() {
    canvas = document.getElementById('selector-canvas') as HTMLCanvasElement;
    if (!canvas) {
    ...
    if (window.selectorAPI && window.selectorAPI.receive) {
        window.selectorAPI.receive('set-background', (dataUrl: string, receivedScaleFactor: number) => {

...
let mainWindow: Electron.BrowserWindow | null; // Keep a reference to the main window
let selectorWindow: Electron.BrowserWindow | null;
...
ipcMain.on('save-screenshot', async (event: Electron.IpcMainEvent, dataURL: string) => {
...
let fullScreenShotImage: Electron.NativeImage | null = null;
...
async function initiateScreenshotSelectionProcess(event?: Electron.IpcMainEvent) {
  ...
  try {
    ...
    if (event && (event as any).sender) {
        (event as any).sender.send('screenshot-captured', null);
...
ipcMain.on('open-selector-window', (event: Electron.IpcMainEvent) => {
  initiateScreenshotSelectionProcess(event);
});

ipcMain.on('region-selected', async (event: Electron.IpcMainEvent, region: { x: number, y: number, width: number, height: number, scaleFactor: number }) => {

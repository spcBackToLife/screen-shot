...
const applyPixelationEffect = (imageData: ImageData, pixelSize: number): ImageData => {
...
// Define types for annotations and other state
interface Annotation {
    type: 'rect' | 'circle' | 'text' | 'pixelate' | 'pencil'; // Added pencil
    id: string; // Added for key prop and managing annotations
    x: number;
    y: number;
    w?: number; // For rect, pixelate
    h?: number; // For rect, pixelate
...
type Tool = 'rect' | 'circle' | 'text' | 'pixelate' | 'pencil' | null;
...
const App: React.FC = () => {
    ...
    const [drawingColor, setDrawingColor] = useState<string>('#FF0000'); // Default to red
    ...
    useEffect(() => {
        if (window.electronAPI && window.electronAPI.receive) {
            const unlisten = window.electronAPI.receive('screenshot-captured', (dataURL: string | null) => {
    ...
    const handleTakeScreenshot = () => {
        if (window.electronAPI) window.electronAPI.send('open-selector-window');
    };
    const handleTogglePin = () => {
        if (window.electronAPI) window.electronAPI.send('toggle-pin');
    ...
    const selectTool = (tool: Tool) => setCurrentTool(tool);
    ...
    return (
        <div className="App">
            <header className="App-header">
                <h1>Screen Capture (React)</h1>
            </header>
            <div className="controls">
                <button onClick={handleTakeScreenshot}>Take Screenshot</button>
                <button onClick={handleTogglePin}>{isPinned ? 'Unpin' : 'Pin'} Window</button>
                <button onClick={handleSaveScreenshot} disabled={!screenshotDataUrl}>Save Screenshot</button>
                <button onClick={handleClearAnnotations} disabled={annotations.length === 0}>Clear Annotations</button>
                <hr />
                Tools:
                <button onClick={() => selectTool('rect')} className={currentTool === 'rect' ? 'active' : ''}>Rectangle</button>
                <button onClick={() => selectTool('circle')} className={currentTool === 'circle' ? 'active' : ''}>Circle</button>
                <button onClick={() => selectTool('pencil')} className={currentTool === 'pencil' ? 'active' : ''}>Pencil</button>
                <button onClick={() => selectTool('text')} className={currentTool === 'text' ? 'active' : ''}>Text</button>
                <button onClick={() => selectTool('pixelate')} className={currentTool === 'pixelate' ? 'active' : ''}>Pixelate</button>
                <input type="color" value={drawingColor} onChange={(e) => setDrawingColor(e.target.value)} title="Drawing Color"/>
            </div>

            <div className="screenshot-container" style={{ position: 'relative' }}>
                {screenshotDataUrl ? (
                    <img
                        ref={imageRef}
                        src={screenshotDataUrl}
                        alt="Screenshot"
                        onLoad={redrawAllAnnotations} // Redraw when image (re)loads
                ...
                {screenshotDataUrl && (
                    <canvas
                        ...
                        style={{ position: 'absolute', top: 0, left: 0, cursor: currentTool ? 'crosshair' : 'default' }}
                        // Width and height are set in redrawAllAnnotations
                    />
                )}
            </div>
        </div>
